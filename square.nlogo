...
to setup
  ...
  create-turtles pts [
    set id next-id
    set size 1
    setxy (random 42 - random 42)  (random 42 - random 42)
    set next-id (next-id + 1)
  ]
end

to move-by-x-to [target]
  if xcor != target [
    ifelse xcor < target [
      set heading 90 ;; move right
      fd 1
    ] [
      set heading 270 ;; move left
      fd 1
    ]
  ]
end

to move-by-y-to [target]
  if ycor != target [
    ifelse ycor < target [
...
to build-square
  ...
  ask turtles [
    if id >= corner0 and id < corner1 [
      move-by-x-to (-1 * (L - 1) / 2 + id)
      move-by-y-to ((L - 1) / 2)
    ]
    if id >= corner1 and id < corner2 [
      move-by-x-to ((L - 1) / 2)
      move-by-y-to ((L - 1) / 2 - (id - corner1))
    ]
    if id >= corner2 and id < corner3 [
      move-by-x-to ((L - 1) / 2 - (id - corner2))
      move-by-y-to (-1 * (L - 1) / 2)
    ]
    if id >= corner3 [
      move-by-x-to (-1 * (L - 1) / 2)
      move-by-y-to (-1 * (L - 1) / 2 + (id - corner3))
    ]
  ]
end
